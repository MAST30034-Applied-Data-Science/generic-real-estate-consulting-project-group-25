...
def linear_imp(cleaned_monetary_data, write=False):
    ...
    :param cleaned_monetary_data: dataframe containing cleaned monetary data
    :param write: save to file ../data/curated/all_imputed_monetary_data.csv or not, defaults to False
    :return: missing values from monetary data are filled
    """
    ...
    for i in range(0, len(cleaned_monetary_data.loc[:, 'Median_age_persons_2011'])):
        ...
        for feature in COL:
            ...
            reg = LinearRegression().fit(X_df.loc[0:2, 'year'].values.reshape(
                (-1, 1)), X_df.loc[0:2, feature].values.reshape((-1, 1)))

            # Predicting new values for missing fields
            years = {2012: -1, 2013: -1, 2014: -1, 2015: -
                     1, 2017: -1, 2018: -1, 2019: -1, 2020: -1}
        ...
        all_yrs = {2011: 0, 2012: 3, 2013: 4, 2014: 5, 2015: 6,
                   2016: 1, 2017: 7, 2018: 8, 2019: 9, 2020: 10, 2021: 2}
        new_row = []
        field = {}
        for c in COL:
    ...
    if (write == True):
        final_rows.to_csv(
            '../data/curated/all_imputed_monetary_data.csv', index=False)
    return final_rows


if __name__ == "__main__":
    cleaned_monetary_data = pd.read_csv(
        '../data/curated/cleaned_monetary_data.csv')
