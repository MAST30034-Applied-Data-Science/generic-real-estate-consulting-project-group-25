...
def addressToCords(address: str):
    ...
    # If there is no matching result, get rid of some non-unique words as sometimes they don't match with the database
        ...
        try:
            query = re.sub(
                "Drive||Street||Avenue||Road||Court||Crescent", "", query)
            url = f"https://nominatim.openstreetmap.org/search?q={query}&format=json&limit=1"
            location = requests.get(url).json()
            return location[0]["lat"], location[0]["lon"]
...
def find_closest_location(rent_lat, rent_long, ext_data, ext_lat_col, ext_lon_col, name_col):
    ...
    :param rent_lat: latitude of starting coordinate
    :param rent_lng: longitude of starting coordinate 
    :param ext_data: a pandas dataframe containing the destinations and their geocordinates
    :param ext_lat_col: column name of ext_data that stores latitude
    :param ext_lng_col: column name of ext_data that stores longitude 
    :param name_col: column name of ext_data that stores the name of the destination
    :return: closest driving distance
    """
    ext_data["cords"] = ext_data[[ext_lat_col,
                                  ext_lon_col, name_col]].values.tolist()
    closest = min(ext_data["cords"], key=lambda x: dist_between_two_lat_lon(
        rent_lat, x[0], rent_long, x[1]))
...
def dist_between_two_lat_lon(lat1, lat2, long1, long2, method="driving-car"):
    ...
    :param method: mode of transport, defaults to "driving-car"
    :return: distance between the two points
    """
    response = requests.get(
        f"http://localhost:8080/ors/v2/directions/{method}?&start={long1},{lat1}&end={long2},{lat2}")
    try:
